...
namespace CSCI444 {
    ...
    public:
        ...
        bool calculateSignedDistanceField(ShaderProgram *computeShader);
         ...
         * @param vertLoc
         */
        void setVertexLocation(GLint vertLoc);
         ...
         * @param idxLoc
         */
        void setIndexLocation(GLint idxLoc);
...
float CSCI444::ModelLoaderSDF::_distTriangle(const Triangle &triangle, const glm::vec3 &point) {
    ...
    glm::vec3 v32 = triangle.v3 - triangle.v2;
    glm::vec3 p2 = point - triangle.v2;
    glm::vec3 v13 = triangle.v1 - triangle.v3;
    glm::vec3 p3 = point - triangle.v3;
    glm::vec3 nor = glm::cross(v21, v13);

    // Get sign of distance
    glm::vec3 origin = (triangle.v1 + triangle.v2 + triangle.v3) / 3.0f;
    float sign = glm::sign(glm::dot(origin - point, nor));
    if (sign == 0.0) {
        sign = 1.0;
    }

    return sign * ((glm::sign(glm::dot(glm::cross(v21, nor), p1)) +
                    glm::sign(glm::dot(glm::cross(v32, nor), p2)) +
                    glm::sign(glm::dot(glm::cross(v13, nor), p3)) < 2.0)
                   ?
                   // 3 edges
                   min(min(
                           _dot2(v21 * glm::clamp(glm::dot(v21, p1) / _dot2(v21), 0.0f, 1.0f) - p1),
                           _dot2(v32 * glm::clamp(glm::dot(v32, p2) / _dot2(v32), 0.0f, 1.0f) - p2)),
                       _dot2(v13 * glm::clamp(glm::dot(v13, p3) / _dot2(v13), 0.0f, 1.0f) - p3))
                   :
                   // 1 face
...
CSCI444::ModelLoaderSDF::calculateSignedDistanceFieldCPU(float resolution, float offset, glm::mat4 initialModelMtx) {
    ...
    for (int i = 0; i < _numIndices; i += 3) {
        ...
        // Get verticies
        triangle.v1 = glm::vec3(_vertices[3 * _indices[i + 0] + 0], _vertices[3 * _indices[i + 0] + 1],
                                _vertices[3 * _indices[i + 0] + 2]);
        triangle.v2 = glm::vec3(_vertices[3 * _indices[i + 1] + 0], _vertices[3 * _indices[i + 1] + 1],
                                _vertices[3 * _indices[i + 1] + 2]);
        triangle.v3 = glm::vec3(_vertices[3 * _indices[i + 2] + 0], _vertices[3 * _indices[i + 2] + 1],
                                _vertices[3 * _indices[i + 2] + 2]);

        // Transform to world space
        triangle.v1 = glm::vec3(initialModelMtx * glm::vec4(triangle.v1, 1.0));
        triangle.v2 = glm::vec3(initialModelMtx * glm::vec4(triangle.v2, 1.0));
        triangle.v3 = glm::vec3(initialModelMtx * glm::vec4(triangle.v3, 1.0));

        // Calculate normal
        glm::vec3 ab = triangle.v2 - triangle.v1;
        glm::vec3 ac = triangle.v3 - triangle.v1;
...
inline bool CSCI444::ModelLoaderSDF::calculateSignedDistanceField(ShaderProgram *computeShader) {
    ...
    if (this->_vertexLoc == -1) {
        fprintf(stderr, "[ERROR]:[SDF]: Vertex buffer location is unset\n");
        return false;
    }
    if (this->_indexLoc == -1) {
        fprintf(stderr, "[ERROR]:[SDF]: Index buffer location is unset\n");
        return false;
    }

    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, this->_sdfLoc, this->_sdfSSBO);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, this->_vertexLoc, this->_vertexSSBO);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, this->_indexLoc, this->_indexSSBO);
...
inline void CSCI444::ModelLoaderSDF::setVertexLocation(GLint vertLoc) {
    this->_vertexLoc = vertLoc;
}

inline void CSCI444::ModelLoaderSDF::setIndexLocation(GLint idxLoc) {
    this->_indexLoc = idxLoc;
}
