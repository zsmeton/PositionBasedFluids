...
#ifndef __CSCI444_SHADEREUTILS_4_H__
#define __CSCI444_SHADEREUTILS_4_H__
...
inline void CSCI444_INTERNAL::ShaderUtils::enableDebugMessages() {
	sDEBUG = true;
}

inline void CSCI444_INTERNAL::ShaderUtils::disableDebugMessages() {
	sDEBUG = false;
}
...
//
////////////////////////////////////////////////////////////////////////////////
inline void CSCI444_INTERNAL::ShaderUtils::readTextFromFile(const char *filename, char* &output){
...
inline const char* CSCI444_INTERNAL::ShaderUtils::GL_type_to_string(GLenum type) {
  switch(type) {
...
inline const char* CSCI444_INTERNAL::ShaderUtils::GL_shader_type_to_string(GLenum type) {
  switch(type) {
...
//
////////////////////////////////////////////////////////////////////////////////
inline void CSCI444_INTERNAL::ShaderUtils::printLog( GLuint handle ) {
	int status;
    int infologLength = 0;
    int maxLength;
    bool isShader;
...
inline void CSCI444_INTERNAL::ShaderUtils::printSubroutineInfo( GLuint handle, GLenum shaderStage ) {
	int params, params2;
	int *params3 = NULL;

	glGetProgramStageiv(handle, shaderStage, GL_ACTIVE_SUBROUTINE_UNIFORMS, &params);
	printf("[INFO]: | GL_ACTIVE_SUBROUTINE_UNIFORMS (%-15s): %5i |\n", CSCI444_INTERNAL::ShaderUtils::GL_shader_type_to_string(shaderStage), params);
	for(int i = 0; i < params; i++ ) {
		...
		params3 = (int*)malloc(params2+sizeof(int));
...
inline void CSCI444_INTERNAL::ShaderUtils::printShaderProgramInfo( GLuint handle ) {
	int params;
	...
	for(int i = 0; i < actual_count; i++ ) {
		...
		else if( shaderType == GL_COMPUTE_SHADER ) hasComputeShader = true;
	}
	...
	for(int i = 0; i < params; i++ ) {
		...
		if( sDEBUG ) printf("[INFO]: |   Used in: %-3s %-7s %-7s %-3s %-4s %-3s  Shader(s) |\n", (vs ? "Ver" : ""), (tcs ? "TesCtrl" : ""), (tes ? "TesEval" : ""), (gs ? "Geo" : ""), (fs ? "Frag" : ""), (cs ? "Com" : ""));
	...
	if( vsCount + tcsCount + tesCount + gsCount + fsCount + csCount > 0 ) {
        ...
        if( hasComputeShader ) {
            GLint maxFragmentUniformBlocks = 0;
            glGetIntegerv( GL_MAX_COMPUTE_UNIFORM_BLOCKS, &maxFragmentUniformBlocks );

            if( sDEBUG ) printf( "[INFO]: |  Compute Shader Uniform Blocks: %16d/%2d  |\n", csCount, maxFragmentUniformBlocks );
        }
	}
	...
	if( sDEBUG ) {
		...
		if( major >= 4 ) {
            ...
            if( hasComputeShader ) printSubroutineInfo( handle, GL_COMPUTE_SHADER );
		}
		printf( "[INFO]: \\--------------------------------------------------------/\n\n");
	}
}
...
//
////////////////////////////////////////////////////////////////////////////////
inline GLuint CSCI444_INTERNAL::ShaderUtils::compileShader( const char *filename, GLenum shaderType ) {
...
#endif // __CSCI444_SHADEREUTILS_4_H__
